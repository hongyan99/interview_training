...
// This is NQueen2 optimized based on the mirroring property of the problem
public class NQueen3 {
	...
	private static String[][] arrange(int n) {
		...
		for(int col=0; col<(n+1)/2; col++) {
			...
			solve(1, b, r);
			result.addAll(r);
			if(col==n/2) continue; // no mirroring for middle column
			r.forEach(rr-> {
				String[] rrr = new String[rr.length];
				for(int p = 0; p<n; p++) {
					rrr[p] = new StringBuilder(rr[p]).reverse().toString();
				}
				result.add(rrr);
			});
		}
