...
public class Dijkstra {
	public static void main(String[] args) {
		...
		result.forEach(e-> {
			System.out.print(e.getDistance()+", ");
	...
	public static List<WeightedEdge> mst(List<WeightedEdge> edges, int dim) {
		...
		addEdgesFor(0, root.getSource(), adjListMap, captured, queue);
		...
		while(!queue.isEmpty()) {
			...
			if(edge!=null) {
				result.add(edge);
				addEdgesFor(edge.getDistance(), edge.getDest(), adjListMap, captured, queue);
			}
		}
		return result;
	}
